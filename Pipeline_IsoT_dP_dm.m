...
% Pipe inlet pressure
% Var1 = {6e6, 7e6, 8e6, 9e6, 10e6};
Var1 = {7e6};
% Pipe diameter mm
% Var2 = {0.450,0.900,1.200}; Q = {3*1000000/(24*3600), 14*1000000/(24*3600), 25*1000000/(24*3600) };
% Var = {0.900,1.200};
% Var = {0.9};
% Flow rate
Var2 = {10*1000000/(24*3600),15*1000000/(24*3600),20*1000000/(24*3600),25*1000000/(24*3600)};
% Var2 = {14*1000000/(24*3600)};
% Elevation at H2
% Var = {100};
...
for k=1:length(Var1)
    ...
    for j=1:length(Var2)
        ...
        U_erosional = 0;
        P = 0;
        T = T_f;      % Assuming isothermal flow
        L = 0;
        ...
        % rho(i+1) = CP.PropsSI('D','P',P(i+1),'T',T_f,'Air');
        % h(i+1) = CP.PropsSI('H','P',P(i+1),'T',T_f,'Air');
        % s(i+1) = CP.PropsSI('S','P',P(i+1),'T',T_f,'Air');
        % nu(i+1) = CP.PropsSI('V','P',P(i+1),'T',T_f,'Air');
        % Z(i+1) = CP.PropsSI('Z','P',P(i+1),'T',T_f,'Air');
        % u(i+1) = 14.7349*( Q_a_day/D_mm^2 )*( P_a/T_a )*(Z(i+1)*T_f/P(i+1)); % Q_b has to be converted to m3/day and D to mm
        % U_erosional(i) = 1.22*100/sqrt(rho(i)); % Erosional velocity - it is advised that the actual 
                                                    % velocity be up to 50% of the erosional velocity.
                                                    % The constant 100, can be any value from 100 to 250
        psi(i) = h(i)-h0 - T0*(s(i)-s0)+u(i)^2/2;
        ...
        xlabel('L [km]')
        ylabel('P [kPa]') 
        titulo = strcat(string(Var1{k}),'+',string(Var2{j}));
        title(titulo)
