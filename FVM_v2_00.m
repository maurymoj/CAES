...
% L = 5000; % Kiuchi
% D = 0.5;
% L = 70000; % Transmission pipeline
% D = 0.9;
% L = 213333; % Same volume as Huntorf but same diameter as pipeline
% L = 300; % Huntorf cavern
% D = 24;
L = 35;
D = 40;
...
vol_a = 1/rho_a;
u_a = CP.PropsSI('U','P',P_a,'T',T_a,'Air');
...
X = P*(A_h*L).*(P_a./P - 1 + log(P./P_a))./(1e6*3600); % Pressure Exergy [MWh]
X_min = P_o*(A_h*L).*(P_a./P_o - 1 + log(P_o./P_a))/(1e6*3600);
