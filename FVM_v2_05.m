...
for j=2:n_t
    ...
    while count(j) < 100 && max(abs(error_P)) > tol
        ...
        % conditions
        a_M(1,2) = -max( 0, -rho(2,j)*v_n(2,j)/dx);    % a_C
        a_M(1,1) = rho_f(2,j)/dt + f(1)*rho_f(2,j)*abs(v(2,j))/(2*D) ...
            + (rho(2,j)*v_n(2,j) - rho(1,j)*v_n(1,j))/dx ...
            - (a_M(1,2) - max(rho(1,j)*v_n(1,j)/dx,  0) );% a_B
        ...
        a_M(end,end-1) = -max(rho(n_n-1,j)*v_n(n_n-1,j)/dx, 0); % a_N-2 (index N-2, N-3)
        % WHAT TO DO REGARDING TO THE a INDICES ??
        a_M(end,end) = ...                                  % a_N-1 (index N-2, N-2)
            rho_f(n_f-1,j)/dt + f(n_f-1)*rho_f(n_f-1,j)*abs(v(n_f-1,j))/(2*D)...
            +(rho(n_n,j)*v_n(n_n,j) - rho(n_n-1,j)*v_n(n_n-1,j))/dx...
            - (a_M(end,end-1) - max(-rho(n_n,j)*v_n(n_n,j)/dx,  0) );
        
        d_M(end) = -1/dx;
        b_M(end) = (rho_f(n_f-1,j-1)*v(n_f-1,j-1)/dt)-rho_f(n_f-1,j)*g*sind(theta);
        
        B_M(end) = d_M(end)*(P(n_n,j)-P(n_n-1,j)) + b_M(end);
        ...
        % condition 
            a_M(i,i-1) = -max(rho(i,j)*v_n(i,j)/dx,                     0);
            a_M(i,i+1) = -max(                       0, -rho(i+1,j)*v_n(i+1,j)/dx);
            a_M(i,i)   = rho_f(i+1,j)/dt + f(i+1)*rho_f(i+1,j)*abs(v(i+1,j))/(2*D) ...
                - (a_M(i,i-1) + a_M(i,i+1)) ...
                + (rho(i+1,j)*v_n(i+1,j)/dx - rho(i,j)*v_n(i,j)/dx);
            d_M(i) = -1/dx;
            b_M(i) = (rho_f(i+1,j-1)*v(i+1,j-1)/dt)-rho_f(i+1,j)*g*sind(theta);
            
            B_M(i) = d_M(i)*(P(i+1,j)-P(i,j)) + b_M(i);
        end
        ...
        A_C(1,2) = (rho_f(2,j)*d_M(1)/a_M(1,1))/dx - max(0, -drho_dP_f(2)*v_star(2)/dx); % A_2
        ...
        % boundary conditions for matrix/vector in momentum eq.
            A_C(i,i-1) = (rho_f(i,j)*d_M(i-1)/a_M(i-1,i-1))/dx ...
                       - max(drho_dP_f(i)*v_star(i)/dx,                     0);
            A_C(i,i+1) = (rho_f(i+1,j)*d_M(i)/a_M(i,i))/dx ...
        ...
        v_corr(2:end-1)   = d_M./a_i.*(P_corr(2:end)-P_corr(1:end-1));
% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
